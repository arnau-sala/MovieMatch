...
def show_user_stats():
    ...
    st.sidebar.markdown(f"### 📊 Tus Estadísticas", unsafe_allow_html=True)
    ...
    with col3:
        st.metric("📋 Mi Lista", watchlist_count)
...
def show_personalized_recommendations():
    """Mostrar recomendaciones personalizadas con emojis"""
    if st.sidebar.button("🎯 Recomendaciones Personalizadas"):
        st.sidebar.markdown(f"### 🎯 Para Ti", unsafe_allow_html=True)
        
        if st.sidebar.button("🎬 Basado en tus Favoritos"):
            st.info("🔍 Buscando películas similares a tus favoritos...")
            # Aquí iría la lógica de recomendaciones